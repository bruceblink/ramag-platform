...
use ramag_domain::entities::{
    ...
    MqttSubscriptionStatusSink, MqttTransportBackend, MqttTransportCapabilities,
};
    ...
    async fn subscribe(
        ...
        #[cfg(feature = "native")]
        {
            ...
            return run_native_subscription(move || {
                subscribe_native(profile, request, sink, status_sink, commands, cancelled)
            })
            .await;
        }
        #[cfg(not(feature = "native"))]
        {
            let _ = (profile, request, sink, status_sink, commands, cancelled);
