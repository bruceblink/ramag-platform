...
- 修复 Windows Release 在源码规模检查阶段失败的问题，将结果差异筛选、MQTT Role 管理操作拆分到独立源文件。
- 保留 `v0.1.1` 已推送 tag 不变，使用包含修复的新版本 tag 发布，避免远端 tag 指向不完整构建。

### 构建

- 确保三平台发布任务执行前通过 Rust 源文件行数检查，继续使用同一版本号生成安装包、更新清单和 SHA-256 清单。

## [0.1.1] - 2026-09-15

### 优化

- 数据库结果差异视图增加“全部、修改、新增、删除”筛选，修改项同时保留基准行和当前行。
- 数据库表结构差异、执行计划、结果差异和迁移预览继续优化紧凑窗口下的工具栏与信息展示。

### 修复

- 修复数据库结果差异筛选后单元格变化区域与行变化状态不一致的问题。
- 修复 GitHub Desktop Release 在生成发布说明后重复创建 `release-assets` 目录导致发布任务失败的问题。

### 构建

- 将本次发布版本统一升级为 `0.1.1`，并保持三平台安装包、更新清单和 SHA-256 清单使用同一版本号。
