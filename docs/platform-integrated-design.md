...
| Kafka 工具 | 元数据、Topic、Partition、消息读取/搜索/生产、消费者组、ACL、Topic/配置管理、`KafkaMonitoringDriver`、实时 Tail、指标快照、Schema Registry 版本浏览和本机真实 Kafka JMX Exporter 链路已存在 | 当前继续使用 `rdkafka/librdkafka`；纯 Rust Transport、生产 exporter 安全配置和真实 Windows 窗口证据仍待补充 | 基础管理、协议观测、单条生产和 Broker 运行指标本机复核已落地 |
