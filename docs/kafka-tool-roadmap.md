...
> 状态：阶段 24 已完成列表重绘、Topic/Partition 与消费者组快照预算、刷新合并和消费者组读取取消；Topic/运行时、ACL、配置等其他读取路径的资源释放仍是未完成项，Docker exporter、真实 Broker 端点和真实 Windows 截图也仍待补充
> 更新日期：2026-09-08
...
> 当前基线：`dev`（阶段 18-24 的高规模列表与快照边界切片已同步，刷新合并和消费者组读取取消已完成，其他读取路径资源释放仍待开发）
...
- 本切片不改变 Kafka 服务端查询上限、消费者组数据约定或 Offset 语义；Partition 快照内存预算、刷新合并和消费者组读取取消已完成，Topic/运行时、ACL、配置等其他读取路径的资源释放仍待后续切片。
