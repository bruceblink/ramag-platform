...
> 状态：现行主线；`SHELL-001`、`DB-RED-01`、`DB-RED-03`、`DB-RED-04`、`DB-RED-05A`、`DB-RED-05B`、`DB-RED-05C`、`DB-RED-05D`、`DB-RED-06`、`DB-RED-07`、`DB-UX-003A`、`DB-UX-003B`、`DB-UX-003C`、`DB-UX-004A`、`DB-UX-004B-1`、`DB-UX-004B-2`、`DB-UX-004B-3A`、`DB-UX-004B-3B` 的代码、Docker 与 headless 验证已完成，当前推进 `DB-UX-005A`；数据库分析完成后进入原生 GPUI 插件标准化和 IT Tools 分步迁移。`DB-UX-004B-3B` 原生窗口证据待环境恢复后补验。
> 更新日期：2026-09-26
...
| `DB-UX-005` | 分析、差异和迁移工作流 | 进行中（`DB-UX-005A`：EXPLAIN ANALYZE 风险确认） | `DB-UX-004` | 原始回退、人工确认、回读和窗口证据 |
