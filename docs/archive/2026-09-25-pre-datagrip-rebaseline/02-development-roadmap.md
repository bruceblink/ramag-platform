...
专项文档负责领域细节，本文件负责跨工具排期和当前任务。插件平台见 [`04-plugin-platform-roadmap.md`](../../04-plugin-platform-roadmap.md)，Kafka 见 [`06-kafka-tool-roadmap.md`](../../06-kafka-tool-roadmap.md)，数据库见 [`03-database-client-datagrip-roadmap.md`](03-database-client-datagrip-roadmap.md)，API 测试工具见 [`05-api-testing-tool-roadmap.md`](../../05-api-testing-tool-roadmap.md)，架构边界见 [`architecture.md`](../../architecture.md)。专项文档不得重新定义本文件的跨工具优先级；发现状态冲突时，先修正实现状态和证据，再开始下一项任务。
...
- 当前本机 SQL 集成测试基线为 MySQL 8.4+ 和 PostgreSQL 17+；`scripts/db-test/compose.yaml` 固定使用 `mysql:8.4` 与 `postgres:17-alpine`。历史记录中注明 MySQL 8.0 的测试属于旧基线，不作为当前兼容性或验收证据。
...
| GPUI Kit 迁移 | `GPUI-KIT-001` 已完成；workspace、共享 UI、主程序、工具和测试入口统一使用 `gpui-kit` 0.6.6 | 维护现有 facade 用法；后续版本升级单独评估并补全目标平台验收 | 不把业务功能修复与依赖升级混在同一提交；不以编译通过代替 UI 回归验收 |
...
| `GPUI-KIT-001` | GPUI Kit 迁移 | workspace UI 依赖、`ramag-ui`、`ramag-bin`、共享测试入口和各工具 crate | 已完成 | 当前功能扩展已收敛 | `gpui-kit` 0.6.6、feature、平台初始化、Assets、测试宏和共享 UI 入口完成迁移；workspace 库测试和全目标检查通过；真实平台验收仍按工具专项记录 |
...
`CMT-003` Registry v2 分页切片（2026-09-17）：`ramag-infra-container-registry` 的仓库目录和 Tag 查询现在会沿 `Link: rel="next"` 读取后续页面，并限制总页数、仓库数量、Tag 数量和响应大小；分页链接必须与当前 Registry 使用同一协议、主机和端口，不能把凭据带到外部地址。新增本机 TCP HTTP fixture，覆盖多页仓库、多页 Tag、请求查询参数、仓库身份保持和跨域分页链接拒绝。`cargo test --locked -p ramag-infra-container-registry --all-targets` 通过 6 项，已有 2 项需要真实 Docker Registry 的测试继续忽略；目标 crate Clippy、格式检查和 `git diff --check` 通过，新增 `src/lib.rs` 和 `tests/pagination.rs` 均未超过 600 行。完整源码尺寸脚本仍报告基线已有的 `ramag-bin/src/main.rs`、`ramag-domain/src/entities/container.rs`、`ramag-infra-container-docker/src/lib.rs` 和 `ramag-tool-container/src/view.rs`，本切片未修改这些文件。

`CMT-003` Registry v2 认证与 digest 切片（2026-09-17）：新增本机 TCP HTTP fixture，覆盖带 Basic 凭据的请求、401 认证失败映射和错误信息脱敏；认证失败标记为不可重试，Registry 响应正文中的内容不会进入用户错误。清单查询从 `Docker-Content-Digest` 回读并校验 digest，从响应头读取 `Content-Length`，避免把 `HEAD` 无正文状态误判为零大小；同时保留媒体类型和仓库引用。认证与清单 fixture 共 2 项测试通过，目标 crate 全目标测试共 8 项通过（已有 2 项真实 Registry 测试继续忽略）。取消、清理以及镜像拉取、标记、推送和删除的本机 Registry/Engine 联调仍未完成。

`CMT-003` 镜像操作取消切片（2026-09-17）：`ContainerDriver` 新增 `ContainerOperationCancellation`，`ContainerService` 提供带取消标记的镜像操作入口；Docker 拉取/推送的流式响应、Tag 和 Delete 的单次请求都由 `tokio::select!` 监听取消，取消后返回 `ContainerErrorCategory::Cancelled`，不再继续消费或等待 Docker 响应。`ramag-infra-container-docker` 全目标测试 6 项通过、1 项真实 Engine 测试继续忽略；`ramag-app` 216 项和 `ramag-domain` 190 项通过，目标 Clippy、格式和差异检查通过。当前证据只证明取消边界和取消前不连接 Engine；本机 Engine 实际镜像操作、取消后的状态回读、未完成层清理和真实 Windows UI 仍未完成。
...
`DB-001` 表格结果工作区切片（2026-09-13，历史旧基线）：结果数据区收敛为默认表格视图，移除表格、树形、文本和转置转换按钮及对应状态；保留数据结果/执行计划页签、分页、双轴滚动、单元格查看、复制和编辑。服务端排序重新加载结果时恢复排序前的横向位置；结果过滤允许单个 `WHERE` 表达式包含多个 `AND` 条件，同时继续拒绝多语句输入。`cargo test --locked -p ramag-tool-dbclient --lib` 288 项通过；本机 Docker MySQL 8.0（旧基线，不符合当前 MySQL 8.4+ 要求；`127.0.0.1:13306`）和 PostgreSQL 17-alpine（`127.0.0.1:15432`）的派生表查询实际执行多个 `AND` 条件，分别返回 99,900 和 9 行。真实 Windows Debug 窗口使用系统 Win32 截图和鼠标/键盘输入验证 `bulk_records` 的 100,000 行表格、99,900 行多条件筛选，以及横向滚动后点击 `created_at` 排序仍保持右侧列视口；截图保存在本地 `artifacts/ui-data-functional/`，不纳入源代码提交。该测试仍作为当时功能记录，不代表当前 MySQL 基线验证。

`DB-001` 对象树刷新切片（2026-09-13，历史旧基线）：刷新 schema 时保留已有对象树、展开状态、表缓存和当前选择；刷新期间只在顶部显示状态，旧表行继续可操作，schema 请求失败时显示可重试提示；表请求失败时保留旧表行并以内联状态说明失败。删除的 schema 会清理对应展开状态、列缓存和选择。`cargo test --locked -p ramag-tool-dbclient --lib` 的表树模型和渲染回归测试通过；MSVC Debug 构建产物位于 `target\x86_64-pc-windows-msvc\debug\ramag.exe`。本机 Docker MySQL 8.0（旧基线，不符合当前 MySQL 8.4+ 要求；`ramag-db-test-mysql`，`127.0.0.1:13306`，healthy）实际加载 `ramag_test.bulk_records`，系统截图验证 100,000 行结果在刷新开始、进行中和完成后均未被全屏加载态遮挡；截图保存在本地 `artifacts/ui-data-functional/`，不纳入源代码提交。该测试仍作为当时功能记录，不代表当前 MySQL 基线验证。
...
MQTT-008 本地 Broker 认证配置前置校验（2026-09-17）：关闭匿名连接时要求至少一个有效固定账号，避免启动后所有客户端都被拒绝；校验在领域层完成，基础设施层不会为无可用认证方式的配置绑定监听端口。`ramag-domain` MQTT 实体测试、`ramag-infra-mqtt` native 测试和 `ramag-tool-mqtt` 16 项 headless UI 测试通过；workspace Clippy、格式检查和 `git diff --check` 通过。源码尺寸脚本仍报告分支基线已有的 `ramag-bin/src/main.rs`、`ramag-domain/src/entities/container.rs`、`ramag-infra-container-docker/src/lib.rs`、`ramag-tool-container/src/view.rs`、`ramag-tool-mqtt/src/mqtt_view/profile_state.rs` 和 `ramag-tool-mqtt/src/visual_tests.rs` 超过 600 行，本切片未修改这些超限文件。真实 Windows 窗口证据未新增。
...
MQTT-016 本地 Broker 注入发布切片（2026-09-18）：`MqttLocalServerDriver` 新增结构化发布接口，`MqttService` 复用 `MqttPublishRequest` 校验并记录安全运行日志；Native oximqtt 本地 Broker 用容量为 32 的命令队列接收注入请求，把消息送入订阅路由，并把 Retain 消息写入本地保留存储。`ramag-tool-mqtt` 本地 Broker 页面新增 Topic、Payload、UTF-8/Hex/Base64/组合格式、QoS、Retain 和“注入发布”控件。真实 Native MQTT 客户端测试验证即时消息、User Property、Retain 重新订阅回读；headless 测试验证 UI 请求字段。`ramag-infra-mqtt` native 14 项测试通过且 1 项按环境忽略，默认目标 2 项通过，`ramag-app` MQTT 过滤测试 2 项通过，`ramag-tool-mqtt` 26 项和目标 Clippy 通过，格式检查和 `git diff --check` 通过。目标基础设施全量 Clippy 仍受分支已有的两个 `run_subscription_v311/v5` 参数数量告警影响，使用 `-A clippy::too_many_arguments` 后通过；真实 Windows 窗口、两个独立客户端和 Broker 事件流仍未完成。

MQTT-017 本地 Broker 在线客户端快照切片（2026-09-18）：`MqttLocalServerDriver` 和 `MqttService` 新增在线客户端快照接口；Native oximqtt 从真实在线会话读取 Client ID、用户名、远端地址、连接时间以及每客户端的 Topic Filter、QoS 和 No Local，并把 `online_clients_complete` 设为真实结果标记，Topic 目录继续标记为不完整。`ramag-tool-mqtt` 本地 Broker 页面新增在线客户端刷新、连接信息和订阅列表，客户端快照读取期间禁用冲突操作。Native 测试使用真实订阅客户端回读一条订阅记录；headless 测试覆盖客户端、订阅和发布控件边界。`ramag-domain` MQTT 11 项、`ramag-app` MQTT 2 项、`ramag-infra-mqtt` native 14 项（1 项按环境忽略）、默认 2 项、`ramag-tool-mqtt` 27 项通过；目标 Clippy 和格式检查通过，基础设施全量 Clippy 的既有参数数量告警仍需单独处理。实时事件、强制断开、两个独立客户端和真实 Windows 窗口仍未完成。

MQTT-018 本地 Broker 事件流切片（2026-09-18）：`MqttLocalServerDriver` 和 `MqttService` 新增可取消的有界事件订阅接口；Native oximqtt 通过 Hook 产生客户端连接、成功订阅、取消订阅、客户端发布和客户端断开事件，本地注入发布额外产生 `BrokerPublished` 事件，事件来源不会混淆。事件队列容量固定为 256，队列或界面接收方繁忙时丢弃事件并记录，不无限增长；事件中的消息和客户端字段经过领域层校验，日志不输出载荷。`ramag-domain` 194 项、`ramag-app` 217 项、`ramag-infra-mqtt` native 15 项通过且 1 项按环境忽略，Docker MQTT 集成测试 1 项通过，格式检查和 `git diff --check` 通过。事件流 API 已完成，GPUI 时间线接入、客户端强制断开、两个独立客户端完整回读和真实 Windows 窗口仍未完成。

MQTT-019 MQTT 配置列表状态与事件时间线 UI 验收切片（2026-09-19）：修复 MQTT 配置侧栏文字容器缺少可伸缩宽度导致配置名称和端点只显示省略号的问题；配置行现在显示名称、最近连接测试状态（未测试、测试中、可连接或连接失败）和 Broker 端点，状态点颜色与测试结果一致，保存或修改配置后回到未测试。新增事件时间线 headless 验收，覆盖客户端连接、订阅、客户端发布、Broker 注入发布、断开、清空和停止清理；`ramag-tool-mqtt` 29 项库测试、`cargo build --locked -p ramag-bin`、workspace fmt 和 `git diff --check` 通过。真实窗口截图 `artifacts/ui-data-functional/mqtt-sidebar-status-after-fix.png` 使用系统 Win32 `Graphics.CopyFromScreen` 获取；Computer Use 原生窗口接管不可用，因此截图作为原生窗口外观证据，交互功能以 headless 测试覆盖，不宣称 Computer Use 键盘/鼠标验收；本切片未运行 Docker 或远端 MQTT 集成测试。全工作区测试已完成其余测试输出，但既有 `ramag-tool-dbclient` GPUI 窄弹窗测试进程仍以 `STATUS_STACK_BUFFER_OVERRUN` 异常退出，未将其记为全工作区通过。

MQTT-020 发布、订阅与 Mosquitto 管理 UI 验收切片（2026-09-19）：新增管理页面“读取 Dynamic Security”、用户、Group、Role 保存按钮的 headless 点击测试，确认 UI 将完整对象传给 `MqttService` 和管理驱动；发布/订阅 UI 回归覆盖载荷格式、QoS、Retain、No Local、未保存 Broker 参数、Topic 列表、订阅停止状态和消息时间线。`ramag-tool-mqtt` 30 项库测试、workspace Clippy 和格式检查通过；本机 Docker `ramag-mqtt-test` 使用 `eclipse-mosquitto:2.0.20`、`127.0.0.1:18883`，真实 MQTT 5/3.1.1 发布、订阅、QoS 1、User Property 和空闲取消集成测试 1 项通过。该 Docker 配置仅启用匿名 MQTT 数据面，没有 Dynamic Security 控制插件，因此管理命令只完成 headless 管理驱动验收，未宣称真实 Broker 管理命令通过；Computer Use 原生窗口接管仍不可用，UI 证据以 headless GPUI 交互和既有系统截图为准。

MQTT-021 本地 Broker 主题、连接和队列监测切片（2026-09-19）：本地 `oximqtt` Broker 新增可配置连接上限，并在真实服务端快照中返回当前/峰值/累计连接、活动订阅、发布消息、Retain 消息、观察到的 Topic 目录、每 Topic 发布次数/订阅客户端数/载荷大小，以及事件队列和控制队列深度/容量/丢弃计数；主题目录在达到有界容量时明确标记不完整。`ramag-tool-mqtt` 本地服务页新增连接上限输入、运行指标条、主题目录和在线客户端/订阅关系展示，启动或刷新状态后自动读取快照。真实 Native Broker 连接上限和发布订阅回归 16 项通过（1 项按环境忽略），headless MQTT UI 30 项通过，本机 Docker `ramag-mqtt-test`（`eclipse-mosquitto:2.0.20`，`127.0.0.1:18883`）发布订阅集成 1 项通过；真实 Windows 窗口接管不可用，UI 以 headless 布局和交互证据验收。

API-004 API 工作台 UI 切片（2026-09-19）：新增 `ramag-tool-api` 和 `ApiService`，将 API 工具注册到主窗口 Shell；HTTP 编辑器支持 Method、URL、正文，gRPC 编辑器支持 Endpoint、Service、Method、Metadata 和 Protobuf JSON，响应区显示状态、耗时、大小、Headers/Metadata 和有界正文。请求保存到 API Workspace 默认 Collection，发送使用取消标记和请求代次隔离；为适配 GPUI 后台执行器，HTTP/gRPC 驱动在受控 app worker 中建立 Tokio runtime。`ramag-tool-api` headless GPUI 测试覆盖 360/640/1024/1440 像素布局、协议切换、无服务状态和双协议发送；本机 Docker `ramag-api-http-test`（`ramag-api-http-test:python-3.12.11-alpine-3.22`，`127.0.0.1:18089`）和 `ramag-api-grpc-test`（`ramag-api-grpc-test:rust-1.91.0-bookworm`，`127.0.0.1:18090`）均为 healthy，UI 测试实际收到 HTTP 200 和 gRPC `ok`。Computer Use 返回可控应用列表为空，因此没有真实 Windows 窗口截图或键鼠证据；本切片只宣称 headless UI 与真实 Docker 协议验收。API-005 断言、环境变量和历史仍未完成。
...
API 已保存请求切换修正（2026-09-22）：左侧请求列表改为可点击控件，点击后回填对应 HTTP/gRPC 请求并标记当前请求；切换请求时清理旧响应、断言结果、变量回填、Collection 汇总、gRPC Service 目录和遗留 Metadata，避免不同请求之间互相污染。`ramag-tool-api` 21 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 侧栏窄窗口布局修正（2026-09-22）：360px 以下的 API 工作台侧栏改为占满主体宽度并限制为 `240px` 高度，侧栏内容可滚动；桌面端侧栏保持固定宽度并支持纵向滚动，避免大量已保存请求推出编辑器和响应区。`ramag-tool-api` 21 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 已保存请求恢复修正（2026-09-22）：API 服务启动加载本地 Workspace 后会回填首个已保存请求、环境和历史，不再只显示默认空请求；工作台记录当前请求 ID，侧栏不会因重复名称误标请求，保存已有请求时保留 ID 以维持历史关联。`ramag-tool-api` 21 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 请求列表筛选优化（2026-09-22）：请求侧栏新增有界搜索输入，先对全部已保存请求按 Collection、请求名和协议过滤，再限制渲染数量；列表项显示 `Collection / 协议 / 请求名`，并显示当前匹配数量，避免请求超过 50 条后无法定位或重名请求难以区分。`ramag-tool-api` 22 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 请求列表空状态修正（2026-09-22）：搜索没有匹配项时，侧栏显示明确的“没有匹配当前搜索条件的请求”状态；清空搜索后列表恢复，侧栏渲染拆分为独立模块以保持源文件尺寸限制。`ramag-tool-api` 22 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 响应区页签修正（2026-09-23）：响应区使用 GPUI Kit `TabBar` 将已有正文、Headers/Metadata、耗时和断言/变量分组显示，避免长正文和响应摘要在同一面板连续堆叠；切换请求、协议、发送或运行 Collection 后回到正文页签，并清理上一请求的响应状态。新增两个 headless 交互测试，`ramag-tool-api --all-targets` 24 项测试、目标 Clippy、源码尺寸、格式和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 请求搜索匹配数量修正（2026-09-23）：请求侧栏继续只渲染前 50 条匹配结果，但搜索摘要现在统计全部匹配请求，并在结果超过渲染上限时明确显示“显示前 50 个”，避免用户把渲染上限误认为匹配总数。新增超过 50 条匹配请求的回归测试，`ramag-tool-api --all-targets` 25 项测试、目标 Clippy、源码尺寸、格式和差异检查通过；原生 Windows 窗口操作证据仍未取得。

SSH/对象存储传输浮层窄窗口修正（2026-09-23）：传输行允许状态、路径、进度和取消/重试操作换行，面板收缩到 360px 窗口时不会把长路径或操作按钮推出浮层；新增 SSH 和对象存储传输行边界回归测试，目标 crate 测试、Clippy、源码尺寸、格式和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 请求筛选清除交互优化（2026-09-22）：请求搜索框复用共享 `cleanable_input`，输入关键词时显示带提示的清除按钮，清除后立即恢复全部请求并重新聚焦搜索框。`ramag-tool-api` 22 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

API 请求搜索框布局验收（2026-09-22）：为请求搜索框增加稳定容器和调试选择器，布局测试覆盖 360/640/1024/1440 像素窗口，确认搜索框始终位于 API 侧栏内。`ramag-tool-api` 22 项库测试、Clippy、格式检查、源码行数检查和差异检查通过；原生 Windows 窗口操作证据仍未取得。

`DB-001` 结果表排序渲染测试稳定性修正（2026-09-23）：服务端排序滚动位置测试在最终结果渲染时会触发无关的后台派生视图计算，导致 GPUI 测试调度器偶发报告跨线程唤醒。测试现在为最终结果预置匹配的派生视图缓存，并确认缓存命中后再完成渲染；`ramag-tool-dbclient --lib` 连续 3 次各 315 项通过，`cargo test --workspace --all-targets --quiet` 通过。本次只收敛测试范围，不改变生产计算路径；真实 Windows 窗口证据仍单独记录。

API 工作台紧凑高度布局修正（2026-09-23）：360×240 窗口中，固定高度的请求侧栏曾把工具栏和响应面板推到主体之外。窗口较矮时，API 页面现在启用整页纵向滚动，并为侧栏、请求编辑器和响应区保留可用布局高度；侧栏和各内容区仍可单独滚动。新增 360×240 与 1024×240 headless 边界测试，并用滚轮验证整页确实可滚动；`ramag-tool-api --all-targets` 25 项测试、目标 Clippy 和格式检查通过。真实 Windows 窗口证据仍待单独验收。

API 工作台高度切换滚动位置修正（2026-09-23）：紧凑窗口滚动后放大窗口，根视图会保留旧滚动偏移，导致常规高度下内容仍停在下方。进入或离开紧凑高度布局时，页面滚动位置现在会复位；回归测试覆盖紧凑窗口滚轮滚动后恢复 360×640 窗口。`ramag-tool-api --all-targets` 25 项测试、目标 Clippy 和格式检查通过。

MongoDB 单元格详情弹框紧凑高度修正（2026-09-23）：headless 测试复现 360×240 窗口中详情滚动区固定 400px 并越出视口的问题；详情区现在按弹框可用高度收缩并保留内部滚动，标题和正文都保持在窗口内。MongoDB `--all-targets` 105 项测试、workspace Clippy、格式和差异检查通过；原生 Windows 窗口证据仍待单独验收。
数据库服务端排序测试稳定性修正（2026-09-23）：workspace 全目标测试发现结果重载用例在默认窗口宽度下没有足够的横向滚动范围，首次 GPUI 布局会把测试预设的 `-240px` 偏移裁回零；结果视图更新还可能在测试结束时留下后台任务。测试现在先建立 1200px 列宽、完成首帧布局并确认最大滚动范围，再检查排序和结果重载；每次状态变更后在渲染前放入匹配的结果视图缓存。目标测试连续 5 次及数据库工作台 315 项测试通过；本次不改生产滚动行为。

API 工作区历史读取失败提示修正（2026-09-24）：工作区主体读取成功但执行历史读取失败时，不再把历史静默显示为空列表；侧栏显示明确失败状态和重试入口，同时保留已加载请求、环境和保存能力。Storage 原始错误不会进入界面提示。新增失败、重试和敏感信息不泄露回归测试，`ramag-tool-api --lib` 通过 28 项，workspace 全目标测试通过。
...
SSH 远端能力反馈文案修正（2026-09-24）：连接测试成功后的反馈不再直接显示 `RemoteCapabilityState`、Shell、操作系统、SFTP 通道和路径命名空间的 Rust Debug 文本，改为“可用”“不支持”“被策略阻止”“Windows PowerShell”等可读标签；新增多状态组合回归测试，`ramag-tool-ssh --lib` 84 项测试、目标 Clippy、源码尺寸、格式和差异检查通过。真实 Windows 窗口操作证据仍未取得。

SSH 配置测试反馈窄窗口修正（2026-09-24）：连接测试结果增加自然换行和稳定调试选择器，长能力摘要在 360px 窗口内不会推出取消/保存按钮或越出窗口；新增长反馈 headless 边界测试，`ramag-tool-ssh --lib` 85 项测试、目标 Clippy、源码尺寸、格式和差异检查通过。真实 Windows 窗口操作证据仍未取得。

MQTT 订阅失败原因显示修正（2026-09-24）：订阅状态行现在显示 Broker 返回的有界失败原因，不再只显示“失败”；原因最多显示 256 个字符，窄窗口允许自然换行，避免长错误把 Topic 行推出内容区。新增失败文案和 360/640/1024/1440px 布局回归，`ramag-tool-mqtt --lib` 32 项测试、目标 Clippy、源码尺寸、格式和差异检查通过。真实 Windows 窗口和远端 Broker 拒绝操作证据仍未取得。

API 执行历史清理功能与按钮尺寸修正（2026-09-24）：API 工作台历史标题增加带提示和“清空”文案的按钮；按钮使用标准小尺寸，回归测试确认宽高至少为 24px。清空前使用共享二次确认对话框，确认后调用 `ApiService::clear_history` 删除当前工作区的持久化历史，成功后清理侧栏列表，失败时保留原列表并显示通用提示，不显示 Storage 原始错误。新增 headless 交互测试，实际点击清空按钮并通过确认弹框执行成功和失败路径，同时验证失败提示不泄露 Storage 原始错误；`ramag-tool-api --all-targets` 30 项测试、目标 Clippy、源码尺寸、格式和差异检查通过。真实 Windows 窗口操作证据仍未取得。
