...
- 服务端排序保留横向位置的测试现在先建立 1200px 列宽，完成 GPUI 首帧布局并确认实际滚动范围后再设置偏移；排序重载更新结束时预置对应结果视图缓存，避免后台测试任务在退出时唤醒已关闭的测试调度器。
- 表设计器字段编辑区紧凑高度修正：低矮窗口中减少首屏字段行数，并让字段内容在可用高度内滚动；新增字段操作移到字段区标题行。GPUI 弹框测试覆盖 360×240、360×620 和 1024×620，检查标题、字段可视区、新增按钮和取消/预览操作边界。
- 表设计器 DDL 预览紧凑高度修正：在 360×240 GPUI 弹框测试中，固定 420px 的预览区曾把内容和底部操作推出窗口；预览区现在为弹框标题、顶部工具栏、分区标题和底部操作留出空间，并在剩余区域内滚动。headless 测试覆盖 360×240、360×620 和 1024×620，检查标题、DDL 预览和底部操作区边界。
...
- 历史 Docker 测试（MySQL 8.0，旧基线，不符合当前 MySQL 8.4+ 要求）实测 `SELECT SLEEP(30)` 与 PostgreSQL 17 `SELECT pg_sleep(30)` 均可通过后端 ID 取消，并在限定时间内结束；该记录不是当前版本验收证据。
