...
> 文档状态：`KAFKA-001` 阶段 18 评审稿；当前仅记录现有实现和可复核证据，不表示纯 Rust 客户端已经完成
> 更新日期：2026-09-07
> 适用范围：`ramag-domain`、`ramag-app`、`ramag-infra-kafka`、`ramag-tool-kafka` 和 `ramag-bin`
> 代码基线：`ad13f97`；当前实施分支：`feat/kafka-001-transport-matrix`
...
本矩阵只比较当前 native 客户端基线和纯 Rust 客户端候选路径。应用层已经通过 `KafkaDriver`、`KafkaAdminDriver` 分开读取和管理操作；本阶段不修改这些业务接口，也不把 `rdkafka` 类型暴露到领域层、应用层或 UI 层。
...
| 证据 | 结果 | 说明 |
|---|---|---|
| 默认 feature 静态测试 | 已通过 | `cargo +nightly-2026-04-16-x86_64-pc-windows-gnu test --locked --offline -p ramag-infra-kafka --lib`：5 项通过；覆盖默认不启用 native、TLS/SASL feature gate、输入校验和错误分类 |
