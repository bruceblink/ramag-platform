...
|---|---|---|---|
| 插件平台 | 设计完成，生产代码仍是静态工具装配 | `PLAT-001`：插件描述、版本和能力校验 | 动态 ABI、插件市场、第三方不受信任代码 |
...
|---|---|---|---|---|---|
| `PLAT-001` | 插件平台 | `ramag-domain`、`ramag-app` | 开发中 | 无 | `PluginId`、API 版本、能力集合、设置模式和重复注册校验有单元测试；现有工具顺序和入口不变 |
| `PLAT-002` | 插件平台 | `ramag-app`、`ramag-bin` | 待开始 | `PLAT-001` | 静态插件注册、初始化、失败隔离、逆序关闭和迟到调用拒绝有测试 |
...
`PLAT-001` 完成前不开始 `PLAT-002`；Kafka 的 `KAFKA-001` 完成前不继续实现 Metrics Snapshot 或 Live Message Tail；终端和数据库任务不得借机修改 Kafka 或插件协议。
...
当前下一步：完成 `PLAT-001` 的接口模型和注册校验，先通过 `ramag-domain`/`ramag-app` 单元测试、workspace Clippy、格式和源码检查，再提交并推送该独立功能。
