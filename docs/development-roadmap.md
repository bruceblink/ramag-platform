...
> 状态：阶段 0 文档与基线收敛完成；阶段 1 的插件平台 P0-A、P0-B、PLAT-003 已完成；TERM-001 代码已完成，真实 SSH 端点验收受当前环境限制
> 更新日期：2026-09-07
> 适用范围：插件平台、数据库工作台、Kafka 工作台、SSH/终端工作台以及跨工具质量与构建流程
> 分支策略：`main` 是稳定基线，`dev` 是集成分支，每个独立任务使用一个短期 `feat/<task-id>-<name>` 分支
> 当前交付切片：`TERM-001`；代码和静态验证已完成，真实端点验收待补；`KAFKA-001` 暂不开始
...
| SSH/终端 | `alacritty_terminal + GPUI` PTY 核心、SSH/SFTP 工作区、会话状态、每标签重连和 `-L/-R/-D` 参数模型已有 | 补真实 OpenSSH 端点和真实窗口证据；随后再进入 `KAFKA-001` | 在终端核心内加入 SSH、RDP、VNC、Telnet 或 Serial 协议 |
...
| `TERM-001` | SSH/终端 | `ramag-domain`、`ramag-infra-ssh`、`ramag-tool-ssh` | 代码完成，端点待验收 | `PLAT-003` | 会话状态、重连和 `-L/-R/-D` 参数模型有 OpenSSH 参数测试；真实端点因当前环境没有 `sshd` 暂未完成 |
...
`PLAT-003` 完成后，`TERM-001` 的代码交付已完成；补齐真实 OpenSSH 端点证据前不把它标记为完整验收，也不开始 `KAFKA-001`。Kafka 的 `KAFKA-001` 完成前不继续实现 Metrics Snapshot 或 Live Message Tail；终端和数据库任务不得借机修改 Kafka 或插件协议。
...
`TERM-001` 已补齐会话状态、退出标签、每标签重连和端口转发模型。终端继续使用 `alacritty_terminal + GPUI`，只负责 PTY、ANSI、输入输出、选择、剪贴板和资源回收；SSH 认证、Host Key、SFTP 和 JumpServer 留在 `ramag-infra-ssh` 与 `ramag-tool-ssh`。真实 OpenSSH 端点仍需在具备 `sshd` 的环境中完成。
...
未完成项：本机 Windows 没有 `sshd.exe`，WSL 只有 `openssh-client`；临时 OpenSSH 容器镜像拉取超时，因此 Shell、转发建立/停止、断线重连和 Host Key 错误尚未取得真实服务证据。workspace 全量库测试已启动，但被 `rdkafka-sys` 的 Windows GNU 构建前置条件阻断，错误为缺少 MSYS/MinGW CMake generator，与 TERM-001 源码无关。P0-C 设置与权限接口继续另行排期，不把 headless 结果写成真实窗口验收。
