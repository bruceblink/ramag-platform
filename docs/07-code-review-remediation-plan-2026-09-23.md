...
> 状态：R1-R11 已进入已验证、已推送的 `main`；R3、R4 以及本次临时源分支均已清理。
>
...
6. 审查时 MQTT 订阅消息进入有界 UI 队列后遇到背压会静默丢弃，SSH 远程覆盖提交在新文件已替换成功后可能仍报告失败，Linux 单实例旧 Socket 清理存在并发竞态；R8、R9、R10 已修复。
...
- 位置：`docs/performance.md`、`docs/02-development-roadmap.md`、`docs/03-database-client-datagrip-roadmap.md` 及相关验收记录。
...
- 修复结果（2026-09-25）：使用内核 advisory lock 串行化旧 socket 恢复和重新绑定；锁获取失败时安全退出为 Secondary。WSL Ubuntu 24.04 并发测试确认两个启动者最终恰好一个 Primary、一个 Secondary，旧 socket 替换和激活通知回归均通过。
...
- R10 已完成并推送 `main`：`single_instance_linux.rs` 用 `ramag.sock.lock` 和 `File::lock()` 保护旧 socket 清理与重新 bind，新增并发恢复测试。WSL Ubuntu 24.04 执行 `cargo test --locked -p ramag-bin --bin ramag single_instance::tests -- --nocapture`（5 项通过）；Windows fmt、workspace Clippy、源码尺寸和 `git diff --check` 通过。本切片未启动 Docker，也未新增真实窗口交互证据。
...
3. R10 已用可控并发测试复现并修复 Socket 竞态，采用内核锁保护恢复流程；没有通过增加 sleep 或扩大重试次数掩盖竞态。
