...
`SHELL-001` 已完成并推送；`DB-UX-001` 已完成 `DB-RED-01`、`DB-RED-03`、`DB-RED-04`，`DB-UX-002` 已完成 `DB-RED-05A`、`DB-RED-05B`、`DB-RED-05C`、`DB-RED-05D`、`DB-RED-06`、`DB-RED-07` 的 headless 功能切片，`DB-UX-003A`、`DB-UX-003B`、`DB-UX-003C-1`、`DB-UX-003C-2`、`DB-UX-004B-3B` 和 `DB-UX-005A` 的代码、Docker 与 headless 验证已完成；当前继续推进 `DB-UX-005B`（迁移执行后的目标结构回读校验）。`DB-UX-004B-3B` 的 Computer Use 原生窗口证据仍待环境恢复后补验。未完成的旧 UI-001、M1-M4、R 系列或工具专项事项必须先映射到新的切片 ID，并重新满足统一 UI 标准后才能恢复；不能仅修改状态文字宣称完成。
...
这些项目在设计确认前不改变当前 `DB-UX-005B` 的实现范围，也不代表动态插件、第三方市场或远程协作已经实现。
...
- 未完成项：原生窗口鼠标/键盘流程待 Computer Use 恢复后补验；下一项为 `DB-UX-005B`（迁移执行后的目标结构回读校验）。
