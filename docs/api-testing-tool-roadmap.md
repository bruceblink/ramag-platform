...
API 请求搜索匹配数量修正记录（2026-09-23）：请求侧栏继续限制最多渲染 50 条结果，但搜索摘要改为统计全部匹配请求；超过渲染上限时显示匹配总数和“显示前 50 个”，避免把渲染上限误报为匹配数量。新增超过 50 条匹配请求的回归测试，`ramag-tool-api --all-targets` 通过 25 项测试，目标 Clippy、源码尺寸、格式和差异检查通过。真实 Windows 窗口截图、键盘和鼠标证据仍未取得。
